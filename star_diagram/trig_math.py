        ...
        if np.pi / 2 - AXIS_PRECISION < radians < np.pi / 2 + AXIS_PRECISION:
            ...
            hits.extend(
                [
                    ...
                    if p[1] < start_point[1] and abs(start_point[0] - p[0]) < MATCH_DIST
                ]
            )
            ...
            hits.extend(
                [
                    (i, radians) if get_indices else (p, radians)
                    for i, p in enumerate(points_list)
                    if p[0] > start_point[0] and abs(start_point[1] - p[1]) < MATCH_DIST
                ]
            )
            ...
            x_is_increasing = (
                radians > 3 * np.pi / 2 or radians < np.pi / 2
            )  # going right
...
def reflect_across_line(p, a, b, image_width, image_height):
    ...
    if a[0] == b[0] and a[1] == b[1]:
        # line can't be formed b/c points are identical.
        return None
